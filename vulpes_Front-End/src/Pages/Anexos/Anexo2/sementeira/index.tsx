...
import { ISementCabA2, ISementeA2 } from "../../../../Interfaces/anexos/anexo2/sementeira";
...
export const SementeiraPastagemForm = () => {
  const location = useLocation();


  const [obj_anexo_cabecalho, set_obj_anexo_cabecalho] = useState<ISementCabA2>();
  const [obj_anexo, set_obj_anexo] = useState<ISementeA2>();
  const [open_dialog_tem_a_certeza_que_quer_eliminar, set_open_dialog_tem_a_certeza_que_quer_eliminar] = useState(false);
  const [obj_anexo_lista_cabecalho, set_obj_anexo_lista_cabecalho] = useState<ISementCabA2[]>([]);
  const [obj_anexo_lista, set_obj_anexo_lista] = useState<ISementeA2[]>([]);
  ...
  const handle_delete_anexo = async () => {
    try {
      ...
      let res = await del(
        `delete__reg_anexo_dois_maneio_um/${obj_anexo?.id_sementeira_cabecalho}`
      );
      if (res.status === 200) {
        let lista_aux: any = obj_anexo_lista.filter((el) => {
          if (el.id_sementeira_cabecalho !== obj_anexo?.id_sementeira_cabecalho) {
            return el
          }
        })
        ...
        <table style={{ width: "100%" }}>
            ...
            <TableRow>
              ...
              >
                <Stack direction="row" justifyContent="left">
                  4 - Sementeira/instalação, ressementeira ou melhoria da
                  pastagem permanente
                  <Box margin={-1} padding={0}>
                    <BasicPopover
                      text={
                        "Preencher quando houver necessidade de o beneficiário implementar ações de controlo da vegetação arbustiva nas parcelas candidatas, indicando a sua frequência.\n\nNo caso do regime ecológico «maneio da pastagem permanente», as alterações as intervenções previstas no âmbito da melhoria da composição floristica das pastagens (sementeira, ressementeira ou melhoria da pastagem) devem ser propostas considerando o período de 3 anos.\n\nNo caso da intervenção agroambiental «conservação do solo - pastagens biodiversas, as intervenções que visem a ressementeira ou melhoria da pastagem devem ser propostas considerando que o compromisso assumido é de 5 anos."
                      }
                    />
                  </Box>
                </Stack>
          ...
          <TableBody>
              ...
              <StyledTableHead>
                <Stack direction="row" justifyContent="center">
                  ...
                  <Box margin={-1} padding={0}>
                    <BasicPopover
                      text={
                        "Indicar o tipo de intervenção proposto para melhorar a composição florística da pastagem, quando se verificar que a zona homogénea necessita desse tipo de intervenção: \n- Sementeira (apenas admissível no regime ecológico «maneio da pastagem permanente);\n- Ressementeira;\n- Melhoria da pastagem (sementeira de determinadas espécies que se revelem mais adequadas para o efetivo pecuário, tipo de solo, destino da produção pecuária, etc) ."
                      }
                    />
                  </Box>
                </Stack>
            ...
            {obj_anexo_lista_cabecalho?.map((tab) => {
              if (obj_anexo?.id_sementeira_cabecalho === tab.id_sementeira_cabecalho) {
                return (
                  <>
                    <TableRow key={tab.id_sementeira_cabecalho}>
                      <TableCell colSpan={7}>
                        <Stack direction="row" sx={{ justifyContent: "end" }}>
                          <BarraDeFerramentas
                            mostrarBotaoNovo
