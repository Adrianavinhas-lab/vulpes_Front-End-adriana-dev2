...
interface ImarcoGH2Props {
  tabela: ImarcoGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelamarcoGH2: React.FC<ImarcoGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<ImarcoGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<ImarcoGH2>({
    id_marco_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: ImarcoGH2): Promise<ImarcoGH2> {
    try {
