...
interface IJunhoGH2Props {
  tabela: IJunhoGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJunhoGH2: React.FC<IJunhoGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJunhoGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJunhoGH2>({
    id_Junho_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJunhoGH2): Promise<IJunhoGH2> {
    try {
