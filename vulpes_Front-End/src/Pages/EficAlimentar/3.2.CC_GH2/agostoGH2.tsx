...
export interface IAgostoGH2 {
  id_Agosto_GH2: number;
...
interface IAgostoGH2Props {
  tabela: IAgostoGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaAgostoGH2: React.FC<IAgostoGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAgostoGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IAgostoGH2>({
    id_Agosto_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IAgostoGH2): Promise<IAgostoGH2> {
    try {
