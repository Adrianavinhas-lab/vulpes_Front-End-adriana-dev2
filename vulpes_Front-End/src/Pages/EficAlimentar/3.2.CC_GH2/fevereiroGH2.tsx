...
export interface IfevereiroGH2 {
  id_fevereiro_GH2: number;
...
interface IfevereiroGH2Props {
  tabela: IfevereiroGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelafevereiroGH2: React.FC<IfevereiroGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IfevereiroGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IfevereiroGH2>({
    id_fevereiro_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IfevereiroGH2): Promise<IfevereiroGH2> {
    try {
