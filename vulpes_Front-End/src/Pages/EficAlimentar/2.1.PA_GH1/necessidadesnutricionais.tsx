...
export interface INecessidadesNutriGH1 {
  id_nessec_nutr: number;
...
interface INecessidadesNutricionaisProps {
  tabela: INecessidadesNutriGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const NecessidadesNutriEFGH1: React.FC<INecessidadesNutricionaisProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INecessidadesNutriGH1>>>
  >({});
  const [id] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<INecessidadesNutriGH1>({
    id_nessec_nutr: 0,
  ...
  const handleNewRowsChange = (
    field: keyof INecessidadesNutriGH1,
    value: string | number
  ) => {
  ...
  async function getTabela(): Promise<INecessidadesNutriGH1> {
    try {
