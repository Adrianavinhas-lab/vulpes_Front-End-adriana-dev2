...
export interface IAlimentoCompostoGH1 {
  id_alim_compostoGH1: number;
...
interface INAlimentoCompostoProps {
  tabela: IAlimentoCompostoGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const AlimentoCompostoEFGH1: React.FC<INAlimentoCompostoProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAlimentoCompostoGH1>>>
  >({});
  const [id, setId] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<IAlimentoCompostoGH1>({
  ...
  const handleNewRowsChange = (
    field: keyof IAlimentoCompostoGH1,
    value: string | number
  ) => {
