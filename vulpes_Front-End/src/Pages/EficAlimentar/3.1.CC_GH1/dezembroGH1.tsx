...
export interface IDezembroGH1 {
  id_Dezembro_GH1: number;
...
interface IDezembroGH1Props {
  tabela: IDezembroGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaDezembroGH1: React.FC<IDezembroGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IDezembroGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IDezembroGH1>({
    id_Dezembro_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IDezembroGH1): Promise<IDezembroGH1> {
    try {
