...
export interface INovembroGH1 {
  id_Novembro_GH1: number;
...
interface INovembroGH1Props {
  tabela: INovembroGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaNovembroGH1: React.FC<INovembroGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INovembroGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<INovembroGH1>({
    id_Novembro_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: INovembroGH1): Promise<INovembroGH1> {
    try {
