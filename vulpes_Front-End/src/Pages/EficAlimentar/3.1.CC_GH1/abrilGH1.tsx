...
interface IAbrilGH1Props {
  tabela: IAbrilGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaAbrilGH1: React.FC<IAbrilGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAbrilGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IAbrilGH1>({
    id_Abril_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IAbrilGH1): Promise<IAbrilGH1> {
    try {
