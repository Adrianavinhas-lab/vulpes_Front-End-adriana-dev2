...
interface IMaioGH1Props {
  tabela: IMaioGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaMaioGH1: React.FC<IMaioGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IMaioGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IMaioGH1>({
    id_Maio_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IMaioGH1): Promise<IMaioGH1> {
    try {
