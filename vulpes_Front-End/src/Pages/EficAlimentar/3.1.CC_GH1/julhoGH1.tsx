...
interface IJulhoGH1Props {
  tabela: IJulhoGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJulhoGH1: React.FC<IJulhoGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJulhoGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJulhoGH1>({
    id_Julho_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJulhoGH1): Promise<IJulhoGH1> {
    try {
