...
export interface IAgostoGH1 {
  id_Agosto_GH1: number;
...
interface IAgostoGH1Props {
  tabela: IAgostoGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaAgostoGH1: React.FC<IAgostoGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAgostoGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IAgostoGH1>({
    id_Agosto_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IAgostoGH1): Promise<IAgostoGH1> {
    try {
