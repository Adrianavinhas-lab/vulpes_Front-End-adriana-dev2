...
export interface IAlimentoCompostoGH3 {
  id_alim_compostoGH3: number;
...
interface INAlimentoCompostoProps {
  tabela: IAlimentoCompostoGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const AlimentoCompostoEFGH3: React.FC<INAlimentoCompostoProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAlimentoCompostoGH3>>>
  >({});
  const [id] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<IAlimentoCompostoGH3>({
  ...
  const handleNewRowsChange = (
    field: keyof IAlimentoCompostoGH3,
    value: string | number
  ) => {
