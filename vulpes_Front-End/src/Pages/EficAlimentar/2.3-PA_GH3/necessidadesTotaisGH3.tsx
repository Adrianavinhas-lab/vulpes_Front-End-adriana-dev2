...
export interface INecessidadesNutriTotalGH3 {
  id_nutri_totaisGH3: number;
...
interface INecessidadesTotaisProps {
  tabela: INecessidadesNutriTotalGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const NNutricionaisTotaisEFGH3: React.FC<INecessidadesTotaisProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INecessidadesNutriTotalGH3>>>
  >({});
  const [id] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<INecessidadesNutriTotalGH3>({
  ...
  const handleNewRowsChange = (
    field: keyof INecessidadesNutriTotalGH3,
    value: string | number
  ) => {
  ...
  async function getTabela(): Promise<INecessidadesNutriTotalGH3> {
    try {
  ...
  async function criarTabela(
    tabela: INecessidadesNutriTotalGH3
  ): Promise<INecessidadesNutriTotalGH3> {
    try {
