...
interface IAbrilGH3Props {
  tabela: IAbrilGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaAbrilGH3: React.FC<IAbrilGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAbrilGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IAbrilGH3>({
    id_Abril_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IAbrilGH3): Promise<IAbrilGH3> {
    try {
