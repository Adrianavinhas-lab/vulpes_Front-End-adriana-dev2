...
interface ImarcoGH3Props {
  tabela: ImarcoGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelamarcoGH3: React.FC<ImarcoGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<ImarcoGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<ImarcoGH3>({
    id_marco_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: ImarcoGH3): Promise<ImarcoGH3> {
    try {
