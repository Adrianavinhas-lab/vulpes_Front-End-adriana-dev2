...
export interface IfevereiroGH3 {
  id_fevereiro_GH3: number;
...
interface IfevereiroGH3Props {
  tabela: IfevereiroGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelafevereiroGH3: React.FC<IfevereiroGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IfevereiroGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IfevereiroGH3>({
    id_fevereiro_GH3: 0,
  ...
  // async function getTabela(): Promise<IAlimentoCompostoGH3> {
  //   try {
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IfevereiroGH3): Promise<IfevereiroGH3> {
    try {
