...
interface IJulhoGH3Props {
  tabela: IJulhoGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJulhoGH3: React.FC<IJulhoGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJulhoGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJulhoGH3>({
    id_Julho_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJulhoGH3): Promise<IJulhoGH3> {
    try {
