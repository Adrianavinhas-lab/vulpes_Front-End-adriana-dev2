...
export interface IAgostoGH3 {
  id_Agosto_GH3: number;
...
interface IAgostoGH3Props {
  tabela: IAgostoGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaAgostoGH3: React.FC<IAgostoGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAgostoGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IAgostoGH3>({
    id_Agosto_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IAgostoGH3): Promise<IAgostoGH3> {
    try {
