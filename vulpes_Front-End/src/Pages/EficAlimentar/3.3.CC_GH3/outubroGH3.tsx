...
export interface IOutubroGH3 {
  id_Outubro_GH3: number;
...
interface IOutubroGH3Props {
  tabela: IOutubroGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaOutubroGH3: React.FC<IOutubroGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IOutubroGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IOutubroGH3>({
    id_Outubro_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IOutubroGH3): Promise<IOutubroGH3> {
    try {
