...
export interface INecessidadesNutriGH2 {
  id_nessec_nutrGH2: number;
...
interface INecessidadesNutricionaisProps {
  tabela: INecessidadesNutriGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const NecessidadesNutriEFGH2: React.FC<INecessidadesNutricionaisProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INecessidadesNutriGH2>>>
  >({});
  const [id] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<INecessidadesNutriGH2>({
    id_nessec_nutrGH2: 0,
  ...
  const handleNewRowsChange = (
    field: keyof INecessidadesNutriGH2,
    value: string | number
  ) => {
  ...
  async function getTabela(): Promise<INecessidadesNutriGH2> {
    try {
          ...
          <table>
            <TableHead>
              <TableRow>
                ...
                >
                  2. Necessidades Nutricionais e Dieta (por dia e CN)
                </TableCell>
                {editingId === id ? (
                  <TableCell colSpan={8}>
                    <ButtonCadernos
