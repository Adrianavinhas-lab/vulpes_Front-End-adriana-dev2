...
export interface INecessidadesNutriGH3 {
  id_nessec_nutrGH3: number;
...
interface INecessidadesNutricionaisProps {
  tabela: INecessidadesNutriGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const NecessidadesNutriEFGH3: React.FC<INecessidadesNutricionaisProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INecessidadesNutriGH3>>>
  >({});
  const [id] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<INecessidadesNutriGH3>({
    id_nessec_nutrGH3: 0,
  ...
  const handleNewRowsChange = (
    field: keyof INecessidadesNutriGH3,
    value: string | number
  ) => {
  ...
  async function getTabela(): Promise<INecessidadesNutriGH3> {
    try {
