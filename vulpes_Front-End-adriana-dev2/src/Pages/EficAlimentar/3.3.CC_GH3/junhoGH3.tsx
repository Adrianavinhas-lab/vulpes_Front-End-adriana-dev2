...
interface IJunhoGH3Props {
  tabela: IJunhoGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJunhoGH3: React.FC<IJunhoGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJunhoGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJunhoGH3>({
    id_Junho_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJunhoGH3): Promise<IJunhoGH3> {
    try {
