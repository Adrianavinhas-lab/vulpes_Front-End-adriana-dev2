...
export interface IDezembroGH3 {
  id_Dezembro_GH3: number;
...
interface IDezembroGH3Props {
  tabela: IDezembroGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaDezembroGH3: React.FC<IDezembroGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IDezembroGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IDezembroGH3>({
    id_Dezembro_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IDezembroGH3): Promise<IDezembroGH3> {
    try {
