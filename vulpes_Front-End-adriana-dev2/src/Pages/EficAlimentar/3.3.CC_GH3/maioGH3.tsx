...
interface IMaioGH3Props {
  tabela: IMaioGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaMaioGH3: React.FC<IMaioGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IMaioGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IMaioGH3>({
    id_Maio_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IMaioGH3): Promise<IMaioGH3> {
    try {
