...
export interface ISetembroGH3 {
  id_Setembro_GH3: number;
...
interface ISetembroGH3Props {
  tabela: ISetembroGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaSetembroGH3: React.FC<ISetembroGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<ISetembroGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<ISetembroGH3>({
    id_Setembro_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: ISetembroGH3): Promise<ISetembroGH3> {
    try {
