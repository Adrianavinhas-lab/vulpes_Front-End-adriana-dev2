...
export interface INovembroGH3 {
  id_Novembro_GH3: number;
...
interface INovembroGH3Props {
  tabela: INovembroGH3;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaNovembroGH3: React.FC<INovembroGH3Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INovembroGH3>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<INovembroGH3>({
    id_Novembro_GH3: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: INovembroGH3): Promise<INovembroGH3> {
    try {
