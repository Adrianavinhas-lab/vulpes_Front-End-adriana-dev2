...
interface IJunhoGH1Props {
  tabela: IJunhoGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJunhoGH1: React.FC<IJunhoGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJunhoGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJunhoGH1>({
    id_Junho_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJunhoGH1): Promise<IJunhoGH1> {
    try {
