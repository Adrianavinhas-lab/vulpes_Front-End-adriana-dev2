...
export interface ISetembroGH1 {
  id_Setembro_GH1: number;
...
interface ISetembroGH1Props {
  tabela: ISetembroGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaSetembroGH1: React.FC<ISetembroGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<ISetembroGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<ISetembroGH1>({
    id_Setembro_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: ISetembroGH1): Promise<ISetembroGH1> {
    try {
