...
export interface IfevereiroGH1 {
  id_fevereiro_GH1: number;
...
interface IfevereiroGH1Props {
  tabela: IfevereiroGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelafevereiroGH1: React.FC<IfevereiroGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IfevereiroGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IfevereiroGH1>({
    id_fevereiro_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IfevereiroGH1): Promise<IfevereiroGH1> {
    try {
