...
interface ImarcoGH1Props {
  tabela: ImarcoGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelamarcoGH1: React.FC<ImarcoGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<ImarcoGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<ImarcoGH1>({
    id_marco_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: ImarcoGH1): Promise<ImarcoGH1> {
    try {
