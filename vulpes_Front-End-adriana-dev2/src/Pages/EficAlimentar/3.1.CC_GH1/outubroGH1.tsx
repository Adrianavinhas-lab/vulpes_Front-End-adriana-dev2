...
export interface IOutubroGH1 {
  id_Outubro_GH1: number;
...
interface IOutubroGH1Props {
  tabela: IOutubroGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaOutubroGH1: React.FC<IOutubroGH1Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IOutubroGH1>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IOutubroGH1>({
    id_Outubro_GH1: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IOutubroGH1): Promise<IOutubroGH1> {
    try {
