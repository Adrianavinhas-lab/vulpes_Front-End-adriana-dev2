...
export interface INecessidadesNutriTotalGH1 {
  id_nutri_totais: number;
...
interface INecessidadesTotaisProps {
  tabela: INecessidadesNutriTotalGH1;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const NNutricionaisTotaisEFGH1: React.FC<INecessidadesTotaisProps> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INecessidadesNutriTotalGH1>>>
  >({});
  const [id] = useState(0);

  /************* TABELA EFETIVO PECUARIO ***********************************/
  // const [rowsEfetivo, setRowsEfetivo] =useState<IEfetivoPecuario[]>([]);
  const [efetivo, setEfetivo] = useState<INecessidadesNutriTotalGH1>({
    id_nutri_totais: 0,
  ...
  const handleNewRowsChange = (
    field: keyof INecessidadesNutriTotalGH1,
    value: string | number
  ) => {
  ...
  async function getTabela(): Promise<INecessidadesNutriTotalGH1> {
    try {
  ...
  async function criarTabela(
    tabela: INecessidadesNutriTotalGH1
  ): Promise<INecessidadesNutriTotalGH1> {
    try {
