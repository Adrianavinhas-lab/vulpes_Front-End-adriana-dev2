...
export interface IJaneiroGH2 {
  id_janeiro_GH2: number;
...
interface IJaneiroGH2Props {
  tabela: IJaneiroGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJaneiroGH2: React.FC<IJaneiroGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJaneiroGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJaneiroGH2>({
    id_janeiro_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJaneiroGH2): Promise<IJaneiroGH2> {
    try {
