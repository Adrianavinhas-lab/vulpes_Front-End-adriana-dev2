...
export interface IOutubroGH2 {
  id_Outubro_GH2: number;
...
interface IOutubroGH2Props {
  tabela: IOutubroGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaOutubroGH2: React.FC<IOutubroGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IOutubroGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IOutubroGH2>({
    id_Outubro_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IOutubroGH2): Promise<IOutubroGH2> {
    try {
