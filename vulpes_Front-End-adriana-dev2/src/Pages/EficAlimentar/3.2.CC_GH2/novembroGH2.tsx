...
export interface INovembroGH2 {
  id_Novembro_GH2: number;
...
interface INovembroGH2Props {
  tabela: INovembroGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaNovembroGH2: React.FC<INovembroGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<INovembroGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<INovembroGH2>({
    id_Novembro_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: INovembroGH2): Promise<INovembroGH2> {
    try {
