...
interface IAbrilGH2Props {
  tabela: IAbrilGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaAbrilGH2: React.FC<IAbrilGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IAbrilGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IAbrilGH2>({
    id_Abril_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IAbrilGH2): Promise<IAbrilGH2> {
    try {
