...
interface IMaioGH2Props {
  tabela: IMaioGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaMaioGH2: React.FC<IMaioGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IMaioGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IMaioGH2>({
    id_Maio_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IMaioGH2): Promise<IMaioGH2> {
    try {
