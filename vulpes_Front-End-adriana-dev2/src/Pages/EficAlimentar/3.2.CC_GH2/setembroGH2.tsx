...
export interface ISetembroGH2 {
  id_Setembro_GH2: number;
...
interface ISetembroGH2Props {
  tabela: ISetembroGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaSetembroGH2: React.FC<ISetembroGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<ISetembroGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<ISetembroGH2>({
    id_Setembro_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: ISetembroGH2): Promise<ISetembroGH2> {
    try {
