...
export interface IDezembroGH2 {
  id_Dezembro_GH2: number;
...
interface IDezembroGH2Props {
  tabela: IDezembroGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaDezembroGH2: React.FC<IDezembroGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IDezembroGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IDezembroGH2>({
    id_Dezembro_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IDezembroGH2): Promise<IDezembroGH2> {
    try {
