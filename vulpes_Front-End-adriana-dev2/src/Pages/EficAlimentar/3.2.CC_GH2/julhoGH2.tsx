...
interface IJulhoGH2Props {
  tabela: IJulhoGH2;
  onInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}

export const TabelaJulhoGH2: React.FC<IJulhoGH2Props> = ({
  tabela,
  onInputChange,
}) => {
  ...
  const [, setEditedFields] = useState<
    Partial<Record<number, Partial<IJulhoGH2>>>
  >({});
  const [id, setId] = useState(0);

  const [row, setRow] = useState<IJulhoGH2>({
    id_Julho_GH2: 0,
  ...
  /********* CREATE *******************/
  async function criarTabela(tabela: IJulhoGH2): Promise<IJulhoGH2> {
    try {
